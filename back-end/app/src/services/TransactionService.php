...
class TransactionService {
    ...
    public function saveTransaction($transactionType, $siteType, $eventId, $siteId, $userId, $sectionId, $seatsList, $seatsAmount , $transactionCode){
        ...
        $siteType= intval($siteType);  
        $eventId= intval($eventId);
        $siteId=intval($siteId); 
        $userId=intval($userId); 
        $sectionId= trim($sectionId); 
        $seatsList= $seatsList;
        $seatsAmount= intval($seatsAmount);
        ...
        LoggingService::logVariable($siteId, __FILE__, __LINE__);
         LoggingService::logVariable($sectionId, __FILE__, __LINE__);
								...
								if($this->validation->isValidString($sectionId)){//7
									...
									if($this->validation->isValidInt($seatsAmount)){//8
										...
										if($this->validation->isValidString($transactionCode)){//9
											...
											$result= $this->regiterTransaction($transactionCode, $seatsAmount, $transactionType);
     ...
     private function regiterTransaction($transactionCode, $seatsAmount, $transactionType){
    	$result=[];
...
//getClientId($userId)
    private function getClientId($userId){
    	...
    	$query="SELECT tbcliente.idCliente
				...
				WHERE tbusuario.idUsuario as idUsuario= :id";
